...
# To lowercase
sub_x$token <- lapply(sub_x$token, tolower)
sub_x$lemma <- lapply(sub_x$lemma, tolower)
str(x)
...
sub_x$lemma <- as.character(sub_x$lemma)
sub_x$token <- as.character(sub_x$token)
...
dtm_matrix_colsum_perc_threshold <- dtm_matrix_colsum_perc_sub[dtm_matrix_colsum_perc_sub > 0.5]
  ...
  for(b in beta){
    for(a in alpha){
      lda_model_tuned <- FitLdaModel(dtm = dtm_threshold,
                                     ...
                                     calc_coherence = T,
                                     calc_r2 = T)
      if (mean(lda_model_tuned$coherence) > 0.14) {
...
save.image(file='pre_pos_tagging_lower_italian_01_perc_env.RData')
...
for (i in head(order(cohe_tab$Coherence, decreasing=TRUE),5)){
  ...
  print(c("R2 = ",as.numeric(model_list[[i]]$r2)))
...
hist(model_max$prevalence)
plot(model_max$prevalence, model_max$alpha, xlab = "prevalence", ylab = "alpha")
model_max$alpha
#summary
...
str(topic_year)
for (i in years) {
  new_row_1 <- c(i,"Literature",sum(final_data_topic$t_1[final_data_topic$year == i]))
  new_row_2 <- c(i,"Travels",sum(final_data_topic$t_2[final_data_topic$year == i]))
  new_row_3 <- c(i,"Leisure",sum(final_data_topic$t_3[final_data_topic$year == i]))
  new_row_4 <- c(i,"Relationships",sum(final_data_topic$t_4[final_data_topic$year == i]))
...
nodes <- data.frame(node = c(0:(length(years)+3)), 
                    name = c(years, "Literature", "Travels", "Leisure", "Relationships"))
head(nodes)
...
palette3 <- brewer.pal(n = 6, name = "Set2")
palette3
#"#1B9E77" "#D95F02" "#7570B3" "#E7298A"

#my_color <- 'd3.scaleOrdinal() .domain(["Letteratura", "Viaggiare", "Tempo libero", "Relazioni umane","my_unique_group"]) .range([#1B9E77", "#D95F02", "#7570B3", "#E7298A","grey"])'
#my_color <- 'd3.scaleOrdinal() .domain(["Letteratura", "Viaggiare", "Tempo libero", "Relazioni umane","my_unique_group"]) .range(["red", "blue", "green", "pink","grey"])'
my_color <- 'd3.scaleOrdinal() .range(["#66C2A5", "#FFD92F", "#E78AC3", "#A6D854", "grey"])'
...
sankey<-networkD3::sankeyNetwork(Links = links, Nodes = nodes, 
                         ...
                         fontFamily = "sans-serif", iterations = 0, margin = list(left = 80, right = 80)
                         )
#sankey <- htmlwidgets::prependContent(sankey, htmltools::tags$h1("Topics evolution in time",style = "text-align: center;font-family: sans-serif; font-size: 14px"))
sankey
...
onRender(
  ...
  '
  function(el, x) {
    d3.selectAll(".node text").filter(d => d.name).attr("text-anchor", "begin").attr("x", 50).attr("text-anchor", "end");
  }
  '
)
...
onRender(sankey,
  ...
  function(el) {
    d3.select(el)
      .selectAll(".node text")
      .filter(d => d.name).attr("text-anchor", "begin")
      .attr("x", 40)
...
onRender(
  ...
  d3.select(el)
  .selectAll(".node text")
  .filter(function(d) { return d.name.startsWith("1"); })
...
for (i in 1:nrow(pt_group_df)){
  r1 <- c(pt_group_df$corpus[i],"Literature",pt_group_df$t_1[i])
  r2 <- c(pt_group_df$corpus[i],"Travels",pt_group_df$t_2[i])
  r3 <- c(pt_group_df$corpus[i],"Leisure",pt_group_df$t_3[i])
  r4 <- c(pt_group_df$corpus[i],"Relationships",pt_group_df$t_4[i])
...
nodes <- data.frame(
                    name = c(as.character(person_topic$person), 
                             "Literature", "Travels", "Leisure", "Relationships"
                             )%>% unique()
                    )
...
links$group <- sub(' .*', '', nodes[links$IDsource + 1, 'name'])
...
p <- networkD3::sankeyNetwork(Links = links, Nodes = nodes,
                              ...
                              #sinksRight=FALSE, 
                              fontSize = 12, nodeWidth = 12, 
                              fontFamily = "sans-serif", iterations = 0, margin = list(right = 100),
                              LinkGroup="group", NodeGroup=NULL)
#p <- htmlwidgets::prependContent(p, htmltools::tags$h1("Person-Topic associations",style = "text-align: center;font-family: sans-serif; font-size: 16px"))
p
#p$x$nodes$name <- nodes$name
onRender(
  p,
  '
  function(el, x) {
    d3.selectAll(".node text").attr("text-anchor", "begin").attr("x", -10).attr("text-anchor", "end");
  }
  '
)
